...
fn invoice_and_receipt_pair_dedups_to_single_csv_row() {
    ...
    let csv_text = std::fs::read_to_string(&csv_path).unwrap();
    let lines: Vec<&str> = csv_text.lines().collect();
    // Header + 2 rows (Amazon dedup'd to one, plus B社).
    assert_eq!(lines.len(), 3, "csv: {csv_text}");
    let amazon_rows = lines.iter().filter(|l| l.contains("Amazon")).count();
    assert_eq!(amazon_rows, 1, "Amazon should be one row, got: {csv_text}");
...
fn keep_duplicates_flag_disables_dedup() {
    ...
    let csv_text = std::fs::read_to_string(&csv_path).unwrap();
    let lines: Vec<&str> = csv_text.lines().collect();
    // Header + 2 rows (both Amazon docs kept).
    assert_eq!(lines.len(), 3);
    let amazon_rows = lines.iter().filter(|l| l.contains("Amazon")).count();
    assert_eq!(amazon_rows, 2);
}
